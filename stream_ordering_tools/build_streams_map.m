function streams_matrix = build_streams_map(dem_namefile,extension) 
...
% % -----------------------------------------------------------------------
max_trib_order                  = max_trib_order+1; 
...
% % -----------------------------------------------------------------------
DEM                             = GRIDobj(strcat('inputs/',dem_namefile,extension)); 
...
if (isempty(output_name) == 1)
    output_name_ascii          = strcat('outputs/images/',dem_namefile,underscore,sorting_type,underscore,hierarchy_attribute,underscore,num2str(max_trib_order-1),underscore,num2str(min_drainage_area),underscore, datetime, extension);
    output_name                = strcat('outputs/',dem_namefile,underscore,sorting_type,underscore,hierarchy_attribute,underscore,num2str(max_trib_order-1),underscore,num2str(min_drainage_area),underscore, datetime);
elseif ~(isempty(output_name) == 1)
    output_name_ascii          = strcat('outputs/images/',output_name, extension);
    output_name                = strcat('outputs/',output_name);
end
...
GRIDobj2ascii(ID,strcat('outputs/images/',dem_namefile,underscore,'id',underscore,datetime,extension));

if strcmp(internal_matrices ,'yes')
    GRIDobj2ascii(FDIR,strcat('outputs/images/',dem_namefile,underscore,'flowdir',underscore,datetime,extension));
    GRIDobj2ascii(FA,strcat('outputs/images/',dem_namefile,underscore,'flowacc',underscore,datetime,extension));
    if strcmp(hierarchy_attribute ,'distance')
        GRIDobj2ascii(DISTANCE,strcat('outputs/images/',dem_namefile,underscore,'flowdist',underscore,datetime,extension));
    end
    if strcmp(sorting_type,'horton')
        GRIDobj2ascii(STRAHLER,strcat('outputs/images/',dem_namefile,underscore,'strahler',underscore,datetime,extension));
    end
end

% if strcmp(junctions_points,'yes')
%     GRIDobj2ascii(JUNCTIONS,strcat('outputs/images/',dem_namefile,underscore,'junctions',underscore,datetime,extension));
% end
...
% % -----------------------------------------------------------------------
dlmwrite(strcat(output_name,'.csv'), [x y z' riv_value' acc_value' area_value' id_value' dis_value' jun_value' out_value'], 'precision', 8);


% %exports values to excel with headers 
% % -----------------------------------------------------------------------
% 
% filename = strcat('outputs/',dem_namefile,'_map_',datetime,'.xls');
