function [xy_pourpoint_i, order_pourpoint_i]         = find_pourpoints_cells(flowacc_neighbors, coord_xy_neighbors, strahler_neighbors, order)
...
if ~((isempty(index_value) || (isempty(max_trib_order) == 0 && order == max_trib_order)))  
    ...
    for i = 1:numel(index_value)         
        xy_pourpoint_i(i) = coord_xy_neighbors(index_value(i));
        if strcmp(sorting_type,'hack')
            order_pourpoint_i(i)       = order+1;
        elseif strcmp(sorting_type,'horton')
            order_pourpoint_i(i)       = strahler_neighbors(index_value(i));
        end
    end
end
end
